...
@section('description','新卒、第二新卒向けの就活・インターンサイト【LinkT(リンクト)】の学生向け会員登録フォームです。最初に国籍を選択し、流れに沿って氏名や大学名など基本情報の入力を行えば仮登録が完了します。その後、登録したメールアドレスに送られてくるURLをタップすれば本登録が完了します。')

@section('title','国籍選択│会員登録｜LinkT(リンクト)')
    ...
    <script>
        $(document).ready(function(){
            $(".student").click(function(){
                ...
                var url = '';
                if(input_value == 1){
                    url = "{{route('front.member-entry.one')}}";
                }else if(input_value == 2){
                    url = "{{route('front.overseas-member-entry.one')}}";
                }
