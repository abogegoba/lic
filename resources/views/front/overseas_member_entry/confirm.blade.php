...
@section('content')
    ...
    <div id="main__content">
        <form class="form__confirm">
            ...
            <input type="submit" value="修正" class="form__button js-btn-link" data-href="{{route('front.overseas-member-entry.revise-one')}}">
        </form>
        <form class="form__confirm">
            <dl>
                <dt>国籍</dt>
            ...
            <input type="submit" value="修正" class="form__button js-btn-link" data-href="{{route('front.overseas-member-entry.revise-two')}}">
        </form>
        <form class="form__confirm">
            <dl>
                <dt>学校情報</dt>
                <dd>{{$data->get('schoolType')}}／{{$data->get('facultyType')}}
                    <br>{{$data->get('name')}}
                    <br>{{$data->get('departmentName')}}
                    <br>{{$data->get('graduationPeriodYear')}}
                    年{{$data->get('graduationPeriodMonth')}}月{{$data->get('graduationPeriodTypeLabel')}}
                </dd>
            </dl>
            <input type="submit" value="修正" class="form__button js-btn-link" data-href="{{route('front.overseas-member-entry.revise-three')}}">
        ...
        <form class="form__confirm">
            ...
            <input type="submit" value="修正" class="form__button js-btn-link" data-href="{{route('front.overseas-member-entry.revise-four')}}">
        </form>
        <form class="form__confirm">
            <dl>
                <dt>ID・パスワード</dt>
            ...
            <input type="submit" value="修正" class="form__button js-btn-link" data-href="{{route('front.overseas-member-entry.revise-five')}}">
        ...
        {{Form::open(['url'=>route('front.overseas-member-entry.store')])}}
        <div class="terms invalid-form">
            ...
            <ul>
                ...
                <li><a href="{{route('front.static.privacy')}}">取り扱いについて</a></li>
            </ul>
            <div class="temrs__consent">
                {{Form::checkbox('agreement', 1, '', ["id"=>"agreement"])}}<label for="agreement">会員規約および取り扱いについて同意する</label>
            </div>
            @include('front.common.input_error', ['errors' => $errors, 'targets' => ['agreement']])
        </div>
        <div class="form__controls">
            <input type="button" value="戻る" class="form__controls__prev js-btn-link" data-href="{{route('front.overseas-member-entry.revise-five')}}">
