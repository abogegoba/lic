...
@section('css')
    ...
    <link rel="stylesheet" href="{{asset('css/student-search/detail/index_new.css')}}">
            ...
            <div class="full_width">
                ...
                <div class="half_width">
                    <div class="member_intro">
                        ...
                        <div class="hash_tags">
                            @if(!empty($data->get('member.hashTag.name')))
            ...
            <div class="fixed_tab">
                <form action="{{route('client.message.detail', ['userAccountId' => $data->get('memberUserAccountId')])}}">
                    <button type="submit">
                        <p>この学生に</p>
                        <p>メッセージする</p>
                        <img src="{{asset('img/client/student-search/comment-dots-regular.svg')}}" alt="">
                    </button>
                </form>
            </div>

            {{--<div class="fixed_tab {{ ($data->get('likeDislikeExists') == 1 && $data->get('checkLikeOrNotStatus') == 1) ? 'like_bac' : ''}}" id="like_member" data-member_id="{{$data->get('member.id')}}" data-company_id="{{$data->get('loggedInCompanyAccountId')}}" data-like_id="{{$data->get('likeDislikeExists') == 1 ? $data->get('checkLikeOrNotId') : 0}}" data-like_status="{{$data->get('likeDislikeExists') == 1 ? $data->get('checkLikeOrNotStatus') : 0}}">--}}
            <div class="fixed_tab {{ $data->get('checkLikeOrNotStatus') == 20 ? 'like_bac' : ''}}" id="like_member" data-member_id="{{$data->get('member.id')}}" data-company_id="{{$data->get('loggedInCompanyId')}}" data-like_id="{{$data->get('checkLikeOrNotId') ? $data->get('checkLikeOrNotId') : 0}}" data-like_status="{{$data->get('checkLikeOrNotStatus') ? $data->get('checkLikeOrNotStatus') : 0}}">
                <form>
                    <div id="addLikeMember">
                        @if($data->get('checkLikeOrNotStatus') == 20)
                            <p>気になるリスト</p>
                            <p>に追加しました</p>
                            <img src="{{asset('img/client/student-search/heart-solid.png')}}" alt="">
                        @else
                            <p>気になるリスト</p>
                            <p>に追加</p>
                            <img src="{{asset('img/client/student-search/heart-regular.png')}}" alt="">
                        @endif
                    </div>
                </form>
            </div>
