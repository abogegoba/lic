...
@section('title','基本情報の編集│企業編集｜LinkT(リンクト)')

@section('css')
    <link rel="stylesheet" href="{{asset('css/mypage/common.css')}}">
    <link rel="stylesheet" href="{{asset('css/company-edit/common.css')}}">
    <link rel="stylesheet" href="{{asset('css/company-edit/cropper.min.css')}}">
    <link rel="stylesheet" href="{{asset('css/company-edit/bootstrap_custom.min.css')}}">
@stop
...
@section('js')
    ...
    <script src="{{asset('js/company/cropper.min.js')}}"></script>
    <script>
            ...
            fetch(dataurl)
                ...
                .then(blob => {
                    ...
                    $.ajax({
                        ...
                        url: '{!! route('client.company-edit.basic-information.upload') !!}',
                        data: fd,
                        processData: false,
                        contentType: false,
                        cache: false,
...
@section('vue')
    <script src="{{asset('js/company/basic_information_edit_vue.js')}}"></script>
@stop
...
@section('content')
    ...
    <div class="main__content" id="edit-client" data-upload-url="{{route('client.company-edit.basic-information.upload')}}">
        ...
        <section class="switchTab">
            <ul>
                <li class="active"><a href="{{route('client.company-edit.basic-information')}}">基本情報</a></li>
                <li><a href="{{route('client.company-edit.recruiting.list')}}">求人一覧</a></li>
            </ul>
        </section>
        {{Form::open(['url'=>route('client.company-edit.basic-information.store') ,'id' =>'form'])}}
        <table class="form__table">
            <tr class="invalid-form">
                <th class="required"><label for="name">会社名</label></th>
                <td>
                    {{Form::text('name', $data->get("name"), ["class" => (!empty($errors->has('name')) ? "invalid-control" : ""), "id"=>"name","placeholder" => "例）株式会社infit", "maxlength" => 56])}}
            ...
            <tr class="invalid-form">
                ...
                <td>
                    {{Form::text('nameKana', $data->get("nameKana"), ["class" => (!empty($errors->has('nameKana')) ? "invalid-control" : ""), "id"=>"nameKana","placeholder" => "例）かぶしきがいしゃいんふぃっと", "maxlength" => 56])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['nameKana']])
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td>
                    <div class="address">
                        <div>
                            〒{{Form::tel('zip', $data->get("zip"), ["class" => (!empty($errors->has('zip')) ? "invalid-control" : ""), "id"=>"zip", "placeholder" => "例）1500021", "maxlength" => 7])}}
                            <button type="button" class="button zip js-zip">住所を検索</button>
                            @include('front.common.input_error', ['errors' => $errors, 'targets' => ['zip']])
                        </div>
                        <div class="selectBox__wrapper width100">
                            {{Form::select('prefectures', $data->get("prefectureList"), $data->get("prefectures"), ["class" => (!empty($errors->has('prefectures')) ? "invalid-control" : ""), "id"=>"prefectures", "placeholder" => "都道府県"])}}
                            @include('front.common.input_error', ['errors' => $errors, 'targets' => ['prefectures']])
                        </div>
                        <div>
                            <label for="city">市区町村</label>
                            {{Form::text('city', $data->get("city"), ["class" => (!empty($errors->has('city')) ? "invalid-control width100" : "width100"), "id"=>"city", "maxlength" =>56, "placeholder" => "例）渋谷区恵比寿西2-2-8"])}}
                            @include('front.common.input_error', ['errors' => $errors, 'targets' => ['city']])
                        </div>
                        <div>
                            <label for="room">建物名・階数など</label>
                            {{Form::text('room', $data->get("room"), ["class" => (!empty($errors->has('room')) ? "invalid-control width100" : "width100"), "id"=>"room", "maxlength" =>56, "placeholder" => "例）えびす第2ビル2F"])}}
                            @include('front.common.input_error', ['errors' => $errors, 'targets' => ['room']])
                        </div>
                    </div>
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td>
                    <div class="{{!empty($errors->has('industryCondition.0')) ? 'invalid-form' : ''}}">
                        <select multiple class="form-control invalid-control select2-multiple" name="industryCondition[]">
                            @foreach($data->get("businessTypeList") as $businessTypeKey => $businessTypeVal)
                                @if(old('industryCondition') && empty($errors->has('industryCondition.0')))
                                    @if(in_array($businessTypeKey, old('industryCondition')))
                                        <option selected value="{{$businessTypeKey}}">{{$businessTypeVal}}</option>
                                    @else
                                        <option value="{{$businessTypeKey}}">{{$businessTypeVal}}</option>
                                    @endif
                                @elseif(in_array($businessTypeKey, $data->get("industryCondition")->toArray()))
                                    <option selected value="{{$businessTypeKey}}">{{$businessTypeVal}}</option>
                                @else
                                    <option value="{{$businessTypeKey}}">{{$businessTypeVal}}</option>
                                @endif
                    ...
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['industryCondition.0']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="job-descriptionOfBusiness">事業内容</label></th>
                <td>
                    {{Form::textarea('descriptionOfBusiness', $data->get("descriptionOfBusiness"), ["class" => (!empty($errors->has('descriptionOfBusiness')) ? "invalid-control width100" : "width100"), "id"=>"descriptionOfBusiness", "placeholder" => "例）次世代型就活サイト【LinkT】の企画・開発・運営"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['descriptionOfBusiness']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="establishmentDate">設立</label></th>
                <td>
                    {{Form::text('establishmentDate', $data->get("establishmentDate"), ["class" => (!empty($errors->has('establishmentDate')) ? "invalid-control width100" : "width100"), "id"=>"establishmentDate", "placeholder" => "例）2019年3月5日", "maxlength" => 24])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['establishmentDate']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="capital"></label>資本金</th>
                <td>
                    {{Form::text('capital', $data->get("capital"), ["class" => (!empty($errors->has('capital')) ? "invalid-control width100" : "width100"), "id"=>"capital", "placeholder" => "例）10,000,000円"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['capital']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="payrollNumber">従業員</label></th>
                <td>
                    {{Form::text('payrollNumber', $data->get("payrollNumber"), ["class" => (!empty($errors->has('payrollNumber')) ? "invalid-control width100" : "width100"), "id"=>"payrollNumber", "placeholder" => "例）100人"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['payrollNumber']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="sales">売上高</label></th>
                <td>
                    {{Form::text('sales', $data->get("sales"), ["class" => (!empty($errors->has('sales')) ? "invalid-control width100" : "width100"), "id"=>"sales", "placeholder" => "例）10,000,000円"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['sales']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="representativePerson">代表者</label></th>
                <td>
                    {{Form::text('representativePerson', $data->get("representativePerson"), ["class" => (!empty($errors->has('representativePerson')) ? "invalid-control width100" : "width100"), "id"=>"representativePerson", "placeholder" => "例）代表取締役　後藤剛志","maxlength" => 24])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['representativePerson']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="exectiveOfficers">役員構成</label></th>
                <td>
                    {{Form::textarea('exectiveOfficers', $data->get("exectiveOfficers"), ["class" => (!empty($errors->has('exectiveOfficers')) ? "invalid-control width100" : "width100"), "id"=>"exectiveOfficers", "rows"=>"7", "placeholder" => "例）取締役　中山勢二"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['exectiveOfficers']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="establishment">事業所</label></th>
                <td>
                    {{Form::textarea('establishment', $data->get("establishment"), ["class" => (!empty($errors->has('establishment')) ? "invalid-control width100" : "width100"), "id"=>"establishment","rows"=>"4", "placeholder" => "例）東京都渋谷区恵比寿西2-2-8　えびす第2ビル2F"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['establishment']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="affiliatedCompany">関連会社</label></th>
                <td>
                    {{Form::textarea('affiliatedCompany', $data->get("affiliatedCompany"), ["class" => (!empty($errors->has('affiliatedCompany')) ? "invalid-control width100" : "width100"), "id"=>"affiliatedCompany","rows"=>"4", "placeholder" => "例）株式会社電通"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['affiliatedCompany']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th><label for="qualification">登録・資格</label></th>
                <td>
                    {{Form::textarea('qualification', $data->get("qualification"), ["class" => (!empty($errors->has('qualification')) ? "invalid-control width100" : "width100"), "id"=>"qualification","rows"=>"4", "placeholder" => "例）基本情報技術者"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['qualification']])
                </td>
            </tr>
            <tr class="invalid-form">
                {{--<th class="required"><label for="homePageUrl">ホームページURL</label></th>--}}
                <th><label for="homePageUrl">ホームページURL</label></th>
                <td>
                    {{Form::text('homePageUrl', $data->get("homePageUrl"), ["class" => (!empty($errors->has('homePageUrl')) ? "invalid-control width100" : "width100"), "id"=>"homePageUrl","placeholder" => "例）https://linkt.jp", "maxlength" => 255])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['homePageUrl']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th><label for="recruitmentUrl">採用ホームページ</label></th>
                <td>
                    {{Form::text('recruitmentUrl', $data->get("recruitmentUrl"), ["class" => (!empty($errors->has('recruitmentUrl')) ? "invalid-control width100" : "width100"), "id"=>"recruitmentUrl","placeholder" => "例）https://linkt.jp", "maxlength" => 255])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['recruitmentUrl']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="mainClient">主要取引先</label></th>
                <td>
                    {{Form::textarea('mainClient', $data->get("mainClient"), ["class" => (!empty($errors->has('mainClient')) ? "invalid-control width100" : "width100"), "id"=>"mainClient","rows"=>"4", "placeholder" => "例）株式会社長崎放送"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['mainClient']])
                </td>
            </tr>
            {{--<tr v-cloak class="invalid-form">
                ...
                <td>
                    <div class="logo">
                        ...
                        <input type="file" id="logo" v-bind:class="{'invalid-control':logoErrors.uploadImage}" v-on:change="changeLogo" data-data="{{$data->toJsonLogo()}}">
                    ...
                    @include('front.common.vue_input_error', ['target' => 'logoErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['logo', 'uploadedLogoName', 'uploadedLogoPath']])
                </td>
            </tr>--}}
            <tr v-cloak class="invalid-form">
                <th class="required"><label for="logo">企業ロゴ</label></th>
                <td>
                    <div class="logo">
                        ...
                        {{--<picture id="logoPreview" style="display: none"></picture>--}}
                        <input type="file" id="logo" data-data="{{$data->toJsonLogo()}}">
                        {{--<input type="file" id="logo" v-bind:class="{'invalid-control':logoErrors.uploadImage}" v-on:change="changeLogo" data-data="{{$data->toJsonLogo()}}">--}}
                        <label for="logo">写真を選択</label>
        ...
        <table class="form__table">
            <tr class="invalid-form">
                ...
                <td>
                    {{Form::text('picName', $data->get("picName"), ["class" => (!empty($errors->has('picName')) ? "invalid-control width100" : "width100"), 'id'=>"picName",'placeholder'=>'例）後藤剛志', "maxlength" => 24])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['picName']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="picPhoneNumber">連絡先電話番号</label></th>
                <td>
                    {{Form::tel('picPhoneNumber', $data->get("picPhoneNumber"), ["class" => (!empty($errors->has('picPhoneNumber')) ? "invalid-control width100" : "width100"), 'id'=>"picPhoneNumber",'placeholder'=>'例）0364449999', "maxlength" => 15])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['picPhoneNumber']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="picEmergencyPhoneNumber">緊急連絡先電話番号</label></th>
                <td>
                    {{Form::tel('picEmergencyPhoneNumber', $data->get("picEmergencyPhoneNumber"), ["class" => (!empty($errors->has('picEmergencyPhoneNumber')) ? "invalid-control width100" : "width100"), 'id'=>"picEmergencyPhoneNumber",'placeholder'=>'例）0364449999', "maxlength" => 15])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['picEmergencyPhoneNumber']])
                </td>
            </tr>
            <tr class="invalid-form">
                <th class="required"><label for="picMailAddress">連絡先メールアドレス</label></th>
                <td>
                    {{Form::email('picMailAddress', $data->get("picMailAddress"), ["class" => (!empty($errors->has('picMailAddress')) ? "invalid-control width100" : "width100"), "id"=>"picMailAddress", "maxlength" =>255,'placeholder'=>'例）example@linkt.jp'])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['picMailAddress']])
                </td>
            </tr>
        </table>
        ...
        <table class="form__table">
                    ...
                    <additional id="file-company-image" data="{{$data->toJsonCompanyImages($errors)}}" :max="3" ref="companyImage" :initial-add="true">
                        <template slot="field" slot-scope="component">
                            <div v-for="(file, index) in component.data" class="file" :key="index">
                                ...
                                <button v-bind:id="'companyImageRemove_' + index" type="button" class="delete" v-if="file.url" v-on:click="component.remove(index)">写真を削除</button>
                                {{Form::hidden('companyImageNames[]',null,["v-bind:value" => "file.name"])}}
                                {{Form::hidden('companyImagePaths[]',null,["v-bind:value" => "file.path"])}}
                                @include('front.common.vue_input_error', ['target' => 'file.fileError'])
                            </div>
                            <button type="button" class="add" v-if="component.addable" v-on:click="component.add()">写真を追加</button>
                        </template>
                    </additional>
                    @include('front.common.vue_input_error', ['target' => 'companyImageErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['displayImage']])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['companyImage', 'companyImageNames', 'companyImagePaths']])
                </td>--}}
                <td v-cloak id="company-image" >
                    <additional id="file-company-image" data="{{$data->toJsonCompanyImages($errors)}}" :max="3" ref="companyImage" :initial-add="true">
                        <template slot="field" slot-scope="component">
                            <div v-for="(file, index) in component.data" class="company_file file" v-bind:id="'selectFile_'+index" v-bind:counter="index" :key="index">
                                <div v-bind:id="'selectBox_'+index" v-if="file.url" {{!empty($errors->has('displayImage')) ? 'invalid-form' : ''}}>
                    ...
                    <button type="button" id="addClientImageButton" class="add" {{--v-if="component.addable" v-on:click="component.add()"--}}>写真を追加</button>
                    @include('front.common.vue_input_error', ['target' => 'companyImageErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['displayImage']])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['companyImage', 'companyImageNames', 'companyImagePaths']])
                </td>
            ...
            <tr class="invalid-form">
                ...
                <td>
                    {{Form::textarea('introductorySentence', $data->get("introductorySentence"), ["class" => (!empty($errors->has('introductorySentence')) ? "invalid-control width100" : "width100"), "id"=>"introductorySentence","rows"=>"7", "placeholder" => "例）当社は、企業と学生の1:1のマッチングを可能にする次世代型就活サイトLinkTの運営会社です。"])}}
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['introductorySentence']])
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td>
                    ...
                    @include('front.common.vue_input_error', ['target' => 'prVideoErrors.uploadVideo'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['prVideo', 'prVideoName', 'prVideoPath']])
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td class="video__sec">
                    <div v-cloak class="video">
                        ...
                        @include('front.common.vue_input_error', ['target' => 'video5sErrors.uploadVideo'])
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video5s', 'video5sName', 'video5sPath']])
                    </div>
                    ...
                    @include('front.common.vue_input_error', ['target' => 'video5sThumbErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video5sThumb', 'video5sThumbName', 'video5sThumbPath']])
                    <div v-if="video5s.url">
                        <h3>タイトル (最大9文字)</h3>
                        {{Form::text('video5sTitle', $data->get("video5s.title"), ["class" => (!empty($errors->has('video5sTitle')) ? "invalid-control width100" : "width100"), "id"=>"video5sTitle", "placeholder"=>"タイトル", "maxlength" =>9])}}
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video5sTitle']])
                    </div>
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td class="video__sec">
                    <div v-cloak class="video">
                        ...
                        @include('front.common.vue_input_error', ['target' => 'video10sErrors.uploadVideo'])
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video10s', 'video10sName', 'video10sPath']])
                    </div>
                    <div v-cloak class="video-thumbs">
                        <img v-if="video10sThumb.url" v-bind:src="video10sThumb.url" alt="10秒動画サムネイル画像">
                        <input type="file" id="video10sThumb" v-on:change="changeVideo10sThumb" data-data="{{$data->toJsonVideo10sThumb()}}" v-bind:class="{'invalid-control':video10sThumbErrors.uploadImage}">
                    ...
                    @include('front.common.vue_input_error', ['target' => 'video10sThumbErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video10sThumb', 'video10sThumbName', 'video10sThumbPath']])
                    <div v-if="video10s.url">
                        <h3>タイトル (最大9文字)</h3>
                        {{Form::text('video10sTitle', $data->get("video10s.title"), ["class" => (!empty($errors->has('video10sTitle')) ? "invalid-control width100" : "width100"), "id"=>"video10sTitle", "placeholder"=>"タイトル", "maxlength" =>9])}}
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video10sTitle']])
                    </div>
                </td>
            </tr>
            <tr class="invalid-form">
                ...
                <td class="video__sec">
                    <div v-cloak class="video">
                        ...
                        @include('front.common.vue_input_error', ['target' => 'video15sErrors.uploadVideo'])
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video15s', 'video15sName', 'video15sPath']])
                    </div>
                    <div v-cloak class="video-thumbs">
                        <img v-if="video15sThumb.url" v-bind:src="video15sThumb.url" alt="15秒動画サムネイル画像">
                        <input type="file" id="video15sThumb" v-on:change="changeVideo15sThumb" data-data="{{$data->toJsonVideo15sThumb()}}"  v-bind:class="{'invalid-control':video15sThumbErrors.uploadImage}">
                    ...
                    @include('front.common.vue_input_error', ['target' => 'video15sThumbErrors.uploadImage'])
                    @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video15sThumb', 'video15sThumbName', 'video15sThumbPath']])
                    <div v-if="video15s.url">
                        <h3>タイトル (最大9文字)</h3>
                        {{Form::text('video15sTitle', $data->get("video15s.title"), ["class" => (!empty($errors->has('video15sTitle')) ? "invalid-control width100" : "width100"), "id"=>"video15sTitle", "placeholder"=>"タイトル", "maxlength" =>9])}}
                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['video15sTitle']])
                    </div>
                </td>
            </tr>
            <tr class="invalid-form">
                        ...
                        <additional id="file-feature" data="{{$data->toJsonFeature($errors)}}" :max="6" ref="feature" :initial-add="true">
                            <template slot="field" slot-scope="component">
                                <div v-for="(file, index) in component.data" class="file">
                                    <div>
                                        <video controls="" muted="" poster="" playsinline="" v-if="file.url && file.type === {{ \App\Domain\Entities\CompanyUploadedFile::MOVIE_CONTENT }}">
                                            <source v-bind:src="file.url" class="js-feature-video-pre">
                                            <p>動画を再生するには、videoタグをサポートしたブラウザが必要です。</p>
                                        </video>
                                        <img v-bind:src="file.url" v-if="file.url && file.type === {{ \App\Domain\Entities\CompanyUploadedFile::IMAGE_CONTENT }}" >
                                        ...
                                        <button type="button" class="delete" v-if="file.url && file.type === {{ \App\Domain\Entities\CompanyUploadedFile::MOVIE_CONTENT }}" v-on:click="component.remove(index)">動画を削除</button>
                                        <button type="button" class="delete" v-if="file.url && file.type === {{ \App\Domain\Entities\CompanyUploadedFile::IMAGE_CONTENT }}" v-on:click="component.remove(index)">画像を削除</button>
                                        ...
                                        @include('front.common.vue_input_error', ['target' => 'featureErrors.uploadVideo'])
                                        @include('front.common.vue_input_error', ['target' => 'featureErrors.uploadImage'])
                                        @include('front.common.vue_input_error', ['target' => 'file.fileError'])
                                        @include('front.common.input_error', ['errors' => $errors, 'targets' => ['feature', 'featureNames', 'featurePaths']])
                                    </div>
                                    <div>
                                        <h3>タイトル (最大24文字)</h3>
                                        {{Form::text('featureTitles[]', '', ["class" => "width100", "v-bind:class" => "{'invalid-control':file.titleError}", "v-bind:id" => "'feature-title_' + index", "placeholder"=>"紹介動画のタイトルをご記載ください。", "v-bind:value"=>"file.title", "v-on:input"=>"setFeatureTitle", "maxlength" =>24])}}
                                        @include('front.common.vue_input_error', ['target' => 'file.titleError'])
                                    </div>
                                    <div>
                                        <h3>説明文</h3>
                                        {{Form::textarea('featureDescriptions[]', '', ["class" => "width100", "v-bind:class" => "{'invalid-control':file.descriptionError}", "v-bind:id" => "'feature-description_' + index", "placeholder" => "紹介動画の説明文をご記載ください。","rows"=>"10", "v-bind:value"=>"file.description", "v-on:input"=>"setFeatureDescription"])}}
                                        @include('front.common.vue_input_error', ['target' => 'file.descriptionError'])
                                    </div>
                                </div>
            ...
            <tr class="invalid-form">
                ...
                <td>
                    <div class="flex--ctr">
                        #{{Form::text('hashtag', $data->get("hashtag"), ["class" => (!empty($errors->has('hashtag')) ? "invalid-control width100" : "width100"), "id"=>"hashtag", "placeholder"=>"例）イケメン多数", "maxlength" =>16])}}
                    </div>
                ...
                <td>
                    <div class="flex color">
                        <div class="radio__color">
                            ...
                            <label for="red"><span class="tag--red">{{\App\Domain\Entities\Tag::TAG_COLLAR_CODE_LIST[\App\Domain\Entities\Tag::TAG_COLLAR_RED]}}</span></label>
                        </div>
                        <div class="radio__color">
                            ...
                            <label for="blue"><span class="tag--blue">{{\App\Domain\Entities\Tag::TAG_COLLAR_CODE_LIST[\App\Domain\Entities\Tag::TAG_COLLAR_BLUE]}}</span></label>
                        </div>
                        <div class="radio__color">
                            ...
                            <label for="green"><span class="tag--green">{{\App\Domain\Entities\Tag::TAG_COLLAR_CODE_LIST[\App\Domain\Entities\Tag::TAG_COLLAR_GREEN]}}</span></label>
                        </div>
                        <div class="radio__color">
                            ...
                            <label for="orange"><span class="tag--orange">{{\App\Domain\Entities\Tag::TAG_COLLAR_CODE_LIST[\App\Domain\Entities\Tag::TAG_COLLAR_ORANGE]}}</span></label>
                        </div>
                        <div class="radio__color">
                            ...
                            <label for="purple"><span class="tag--purple">{{\App\Domain\Entities\Tag::TAG_COLLAR_CODE_LIST[\App\Domain\Entities\Tag::TAG_COLLAR_PURPLE]}}</span></label>
                        </div>
                    </div>
        ...
        <p>※アカウントの変更依頼は<a href="{{route('client.contact')}}">お問合せフォーム</a>からお願いします。</p>
        <div class="account">
            @if (!empty($data->get('accountList')))
                @foreach($data->get('accountList') as $account)
                    <div>
                        <div>{{$account->get('name')}}</div>
                        <div>{{$account->get('mailaddress')}}</div>
                        <footer>{{$account->get('lastLoginDatetime')}}</footer>
                    </div>
              @endforeach
            @endif
        </div>
        <div class="form__controls">
            <input type="button" value="登録内容でプレビュー" class="preview js-btn-target-blank" data-href="{{route('client.company-edit.basic-information.preview')}}">
    ...
    @include('client.common.indicator')
@stop
