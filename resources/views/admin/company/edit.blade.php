    ...
    <script>
        ...
        $('#deleteLogo').click(function () {
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: '{!! route('admin.company.delete_logo',['companyId'=>$data->get('companyId')]) !!}',
...
@section('content')
    <h1 class="content-title" id="company-edit" data-upload-url="{{route('admin.upload')}}">企業変更</h1>
    ...
    <div style="margin-top: 2em;">
        {{Form::open(['url'=>route('admin.company.edit', ['companyId'=>$data->get('companyId')])])}}
        <strong>基本情報</strong>
        <table class="table table-form mb-5">
            ...
            <tr>
                ...
                <td class="{{!empty($errors->has('recruitmentUrl')) ? 'invalid-form' : ''}}">
                    {{Form::text('recruitmentUrl', $data->get("recruitmentUrl"), ["class" => "form-control invalid-control","placeholder" => "例）https://linkt.jp"])}}
        ...
        <table class="table table-form mb-5">
            ...
            <tr v-cloak>
                ...
                <td class="invalid-form" id="company-image">
                    <additional id="file-company-image" data="{{$data->toJsonCompanyImages($errors)}}" :max="3" ref="companyImage" :initial-add="true">
                        ...
                        <template slot="field" slot-scope="component">
                            <div v-for="(file, index) in component.data" class="company_file file" v-bind:id="'selectFile_' + index" v-bind:counter="index" :key="index">
                                <div v-bind:id="'selectBox_' + index" class="mb-2 {{!empty($errors->has('displayImage')) ? 'invalid-form' : ''}}" v-if="file.url">
                                ...
                                {{--{{Form::hidden('companyImageNames[]',null,["v-bind:value" => "file.name","v-bind:id" => "'companyImageNames_'+index"])}}
                                {{Form::hidden('companyImagePaths[]',null,["v-bind:value" => "file.path","v-bind:id" => "'companyImagePaths_'+index"])}}--}}
                    ...
                    <div>
                        <button type="button" id="addClientImageButton" class="btn btn-primary w-25" {{--v-if="component.addable" v-on:click="component.add()"--}}>
                            <span>写真を追加</span>
                        </button>
                    </div>
            ...
            <tr v-cloak>
                ...
                <td class="invalid-form">
                    ...
                    <div class="btn-col">
                        <input type="file" id="video5sThumb" v-on:change="changeVideo5sThumb" data-data="{{$data->toJsonVideo5sThumb()}}"
                        ...
                        @foreach($data->get('accountList') as $account)
                            ...
                            <div class="row">
                                <div class="input-group col-sm-6 col-xl-4 {{!empty($errors->has('lastName')) ? 'invalid-form' : ''}}">
                                    ...
                                    {{Form::text('lastName', $account->get('lastName'), ["class" => "form-control invalid-control", "placeholder" => "例）陸", 'maxlength' => 16])}}
                                </div>
                                <div class="input-group col-sm-6 col-xl-4 mt-2 mt-sm-0 {{!empty($errors->has('firstName')) ? 'invalid-form' : ''}}">
                                    ...
                                    {{Form::text('firstName', $account->get('firstName'), ["class" => "form-control invalid-control", "placeholder" => "例）太郎", 'maxlength' => 16])}}
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="input-group col-sm-6 col-xl-4 {{!empty($errors->has('lastNameKana')) ? 'invalid-form' : ''}}">
                                    ...
                                    {{Form::text('lastNameKana', $account->get('lastNameKana'), ["class" => "form-control invalid-control", "placeholder" => "例）りく", 'maxlength' => 16])}}
                                </div>
                                <div class="input-group col-sm-6 col-xl-4 mt-2 mt-sm-0 {{!empty($errors->has('firstNameKana')) ? 'invalid-form' : ''}}">
                                    <div class="input-group-prepend">
                                        <div class="input-group-text input-group-transparent pl-0">名</div>
                                    </div>
                                    {{Form::text('firstNameKana', $account->get('firstNameKana'), ["class" => "form-control invalid-control", "placeholder" => "例）たろう", 'maxlength' => 16])}}
                                </div>
                            </div>
                            ...
                            <div class="{{!empty($errors->has('mailAddress')) ? 'invalid-form' : ''}}">
                                {{Form::email('mailAddress', $account->get('mailaddress'), ["class" => "form-control invalid-control", 'placeholder'=>'例）example@linkt.jp', "maxlength" => 255])}}
                            ...
                            <div class="{{!empty($errors->has('password')) ? 'invalid-form' : ''}}">
                                {{Form::text('password', $account->get('password'), ["class" => "form-control invalid-control", "maxlength" => 14])}}
                                @include('admin.common.input_error', ['errors' => $errors, 'target' => 'password'])
                            </div>
                            <div style="margin-top: 2em;">
                                {{$account->get('lastLoginDatetime')}}
                            </div>
                        @endforeach
                    @endif
                </td>
            ...
            <tr>
                ...
                <td>
                    <div class="{{!empty($errors->has('jobApplicationAvailableNumber')) ? 'invalid-form' : ''}}">
                        @inject('company','\App\Domain\Entities\Company')
