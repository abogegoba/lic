...
<table class="table table-form mb-5">
    ...
    <tr>
        ...
        <td>
            <div class="mb-2 {{!empty($errors->has('englishName')) ? 'invalid-form' : ''}}">
                {{Form::text('englishName',$data->get("englishName"),["id" => "englishName","class"=>"form-control invalid-control","placeholder"=>"例）Riku Taro"])}}
                @include('admin.common.input_error', ['errors' => $errors, 'target' => 'englishName'])
            </div>
        </td>
    </tr>
    @endif
    ...
    <tr>
        ...
        <td>
            ...
            @else
                <div class="mb-2">
                    {{Form::select('country',  $data->get("overseasList"), $data->get("country"), ["class" => "form-control invalid-control"])}}
                    @include('admin.common.input_error', ['errors' => $errors, 'target' => 'country'])
                </div>
                <div class="mb-2">
                    <div class="custom-control-inline {{!empty($errors->has('zipCode')) ? 'invalid-form' : ''}}">
                        {{Form::tel('zipCode', $data->get("zipCode"), ["id"=>"zipCode", "class" => "form-control invalid-control", "placeholder" => "例）100", 'maxlength' => 7])}}
                    </div>
                    @include('admin.common.input_error', ['errors' => $errors, 'target' => 'zipCode'])
                </div>
            @endif
    ...
    <tr>
        <th class="required-icon">
            {{$data->get("country") == 1?'学部系統':'学部名'}}
        </th>
        <td>
            @if($data->get("country") == 1)
                {{Form::select('facultyType',  $data->get("facultyTypeList"), $data->get("facultyType"), ["class" => "form-control invalid-control"])}}
            @else
                {{Form::select('facultyType',  $data->get("overseasFacultyTypeList"), $data->get("facultyType"), ["class" => "form-control invalid-control"])}}
            @endif
