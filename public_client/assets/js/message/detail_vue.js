const app = new Vue({
    ...
    methods: {
        sendMessage: function (url) {
            ...
            vm.errors = [];
            // dispatcher.$emit(dispatcher.EVENT.SHOW_INDICATOR, "作成中...", true);
            let params = new FormData();
            params.append('messageToSend', messageToSend);

            axios.post(url, params).then(function (response) {
                // dispatcher.$emit(dispatcher.EVENT.HIDE_INDICATOR);
                vm.messageLists = response.data.messageLists;
                vm.messageToSend = '';
