...
<?php $__env->startSection('title','メッセージ詳細│メッセージ一覧│マイページ｜LinkT(リンクト)'); ?>
...
<?php $__env->startSection('vue'); ?>
    <script src="<?php echo e(asset('js/message-detail/message_list_vue.js')); ?>"></script>
<?php $__env->stopSection(); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="message-detail">
                ...
                <?php $__currentLoopData = $data->get('exchangeMessageList'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($list->get('sendingUserAccountId') !== $data->get('loggedInUserAccountId') ): ?>
                        <div class="chats chats--company">
                            <div class="round-img">
                                <picture>
                                    <a href=<?php echo e($data->get("companyDetailUrl")); ?>><img src="<?php echo e($list->get('companyLogo')); ?>" alt="企業ロゴ"></a>
                                </picture>
                            </div>
        ...
        <?php if($data->get('exchangeMessageList')): ?>
        <div id="message_send">
            <?php echo e(Form::open(['url'=>route('front.message.detail', ['userAccountId' => $data->get("companyUserAccountId")])])); ?>
            ...
            <div class="chats__form invalid-form">
                ...
                <div class="model_sentence">
                    <?php if($data->get("requestFlg") === true): ?>
                        <?php echo e(Form::textarea('messageToSend', $data->get("request"), ["class" => "width100", "id"=>"messageToSend", "maxlength" => 400, "placeholder" => "メッセージを記入してください"])); ?>

                    <?php else: ?>
                        <?php echo e(Form::textarea('messageToSend', '', ["class" => "width100", "id"=>"messageToSend", "maxlength" => 400, "placeholder" => "メッセージを記入してください"])); ?>

                    <?php endif; ?>
                    <button type="submit" class="model_sentence_button">送信</button>
                    <?php echo $__env->make('front.common.input_error', ['errors' => $errors, 'targets' => ['messageToSend']], \Illuminate\Support\Arr::except(get_defined_vars(), array('__data', '__path')))->render(); ?>
                    <?php echo $__env->make('front.common.business_error',['errors' => $errors, 'target' => 'can_not_send_message'], \Illuminate\Support\Arr::except(get_defined_vars(), array('__data', '__path')))->render(); ?>
                </div>
            </div>
        ...
        $(document).ready(function(){
            ...
            $('#delete_ok').click(function(){
                var data = {
                    ...
                    'receiving_user_account_id' : parseInt('<?php echo e($data->get('companyUserAccountId')); ?>'),
                }

                $.ajax({
                    type: "POST",
                    url: '<?php echo route('front.message.delete-message'); ?>',
                    data: data,
                    cache: false,
                    success: (data) => {
                        if(data == 1){
