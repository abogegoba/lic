...
<?php echo e($data->get('member.lastName')); ?>　<?php echo e($data->get('member.firstName')); ?>のプロフィール｜新卒・第二新卒採用ならLinkT(リンクト)
<?php $__env->stopSection(); ?>

<?php $__env->startSection('css'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('css/student-search/common.css')); ?>">
    <link rel="stylesheet" href="<?php echo e(asset('css/student-search/detail/index_new.css')); ?>">
...
<?php $__env->startSection('js'); ?>
    <script src="<?php echo e(asset('js/student/detail.js')); ?>"></script>
    ...
    <script>
        ...
        var drawer_width = $('#drawer').width();
        var drawer_right_position = (((body_width - container_width)/2))+'px';
        $('#drawer').css('right',drawer_right_position);
        ...
        $(document).ready(function(){
            ...
            $("#like_member").click(function(){
                var that = $(this);
                var member_id = parseInt(that.data('member_id'));
                var company_id = parseInt(that.data('company_id'));
                var like_id = parseInt(that.data('like_id'));
                var like_status = parseInt(that.data('like_status'));
                if(like_status == 0){
                    var data = {
                        member_id : member_id,
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo route('client.like.member'); ?>',
                    data: data,
                    cache: false,
                    success: (data) => {
                        if(data == 1){
        ...
        $(document).scroll(function() {
            ...
            if (window.matchMedia("(max-width: 1281px)").matches)
            {
                ...
                if(current_position > fix_short_info_position){
                    ...
                    var fixed_intro_width = $('.member_details').width();
                    $('#fixed_intro').css('width',fixed_intro_width);
        ...
        /*function position_fixing(current_position) {
            ...
            if(current_position > (maximum_top_position - 80)){
                if (window.matchMedia("(width: 1440px)").matches)
                {
                    top_px = ((student_extension_position.top + 112 )+extra_top)+'px';
                }if (window.matchMedia("(width: 1425px)").matches)
                {
                    top_px = ((student_extension_position.top + 112 )+extra_top)+'px';
                }if (window.matchMedia("(width: 1680px)").matches)
                {
                    top_px = ((student_extension_position.top + 112 )+extra_top)+'px';
                }else{
                    top_px = ((student_extension_position.top -33 )+extra_top)+'px';
                }
            }else{
        ...
        function mobile_position_fixing(current_position) {
            if(current_position >= (maximum_top_position - (footer_height+se.innerHeight()+parseInt(margin_bottom)+25))){
...
<?php $__env->startSection('content'); ?>
    ...
    <section class="contents">
            ...
            <div class="full_width">
                ...
                <div class="half_width">
                    <div class="member_intro">
                        ...
                        <div class="hash_tags">
                            <?php if(!empty($data->get('member.hashTag.name'))): ?>
        ...
        <div class="reload web_view" id="return_button">
            <form action="<?php echo e(route('client.message.detail', ['userAccountId' => $data->get('memberUserAccountId')])); ?>" class="ftr-form">
                <input type="button" class="prev js-btn-link js-key-save" value="戻る" data-href="<?php echo e(route("client.student.list.reload")); ?>">
            </form>
        </div>
        ...
        <div id="student_extension">
            <div class="fixed_return mobile_tab_view">
                <form action="<?php echo e(route('client.message.detail', ['userAccountId' => $data->get('memberUserAccountId')])); ?>" class="mobile_tab_view">
                    <input type="button" class="prev js-btn-link js-key-save" value="戻る" data-href="<?php echo e(route("client.student.list.reload")); ?>">
                </form>
            </div>
            <div class="fixed_tab">
                <form action="<?php echo e(route('client.message.detail', ['userAccountId' => $data->get('memberUserAccountId')])); ?>">
                    <button type="submit">
                        <p>この学生に</p>
                        <p>メッセージする</p>
                        <img src="<?php echo e(asset('img/client/student-search/comment-dots-regular.svg')); ?>" alt="">
                    </button>
                </form>
            </div>

            
            <div class="fixed_tab <?php echo e($data->get('checkLikeOrNotStatus') == 20 ? 'like_bac' : ''); ?>" id="like_member" data-member_id="<?php echo e($data->get('member.id')); ?>" data-company_id="<?php echo e($data->get('loggedInCompanyId')); ?>" data-like_id="<?php echo e($data->get('checkLikeOrNotId') ? $data->get('checkLikeOrNotId') : 0); ?>" data-like_status="<?php echo e($data->get('checkLikeOrNotStatus') ? $data->get('checkLikeOrNotStatus') : 0); ?>">
                <form>
                    <div id="addLikeMember">
                        <?php if($data->get('checkLikeOrNotStatus') == 20): ?>
                            <p>気になるリスト</p>
                            <p>に追加しました</p>
                            <img src="<?php echo e(asset('img/client/student-search/heart-solid.png')); ?>" alt="">
                        <?php else: ?>
                            <p>気になるリスト</p>
                            <p>に追加</p>
                            <img src="<?php echo e(asset('img/client/student-search/heart-regular.png')); ?>" alt="">
                        <?php endif; ?>
                    </div>
                </form>
            </div>
